...
class GridCellSpatialRelationPositionEncoder(PositionEncoder):
    ...
    def __init__(self, coord_dim=2, frequency_num=16,
        ...
        self.input_embed_dim = self.cal_pos_enc_output_dim()
        self.device = device
    ...
    def cal_freq_list(self):
        if self.freq_init == "random":
            ...
            log_timescale_increment = (
                math.log(float(max_radius) / float(min_radius)) / (frequency_num * 1.0 - 1))
...
class GridCellNormSpatialRelationPositionEncoder(GridCellSpatialRelationPositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
    """
    ...
    def __init__(self, coord_dim=2, frequency_num=16,
        ...
        Args:
            ...
            max_radius: the largest context radius this model can handle
        """
        super(GridCellNormSpatialRelationPositionEncoder, self).__init__(
            coord_dim=coord_dim, frequency_num=frequency_num,
            max_radius=max_radius, min_radius=min_radius,
            freq_init=freq_init, device=device)
        self.input_embed_dim = self.cal_pos_enc_output_dim()
        # the frequence we use for each block, alpha in ICLR paper
        self.cal_freq_list()
        self.cal_freq_mat()
...
class HexagonGridCellSpatialRelationEncoder(GridCellSpatialRelationPositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
    ...
    def cal_coord_embed(self, coords_tuple):
        ...
        for coord in coords_tuple:
            for cur_freq in range(self.frequency_num):
                embed.append(
                    math.sin(self.cal_elementwise_angle(coord, cur_freq)))
                embed.append(math.sin(self.cal_elementwise_angle(
...
"""
class TheoryGridCellSpatialRelationEncoder(GridCellSpatialRelationPositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function

    """
    def __init__(self, coord_dim = 2, frequency_num = 16, max_radius = 10000,  min_radius = 1000, freq_init = "geometric", device = "cuda"):
        """
        Args:
...
Learning Grid Cells as Vector Representation of Self-Position Coupled with Matrix Representation of Self-Motion
"""
class TheoryGridCellNormSpatialRelationEncoder(GridCellSpatialRelationPositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function
...
"""
class TheoryDiagGridCellSpatialRelationEncoder(GridCellSpatialRelationPositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function

    """
    def __init__(self, coord_dim = 2, frequency_num = 16, max_radius = 10000, min_radius = 10, dropout = 0.5, f_act = "sigmoid",
...
class NaiveSpatialRelationEncoder(PositionEncoder):
    """
    Given a list of (deltaX,deltaY), encode them using the position encoding function

    """
    def __init__(self, spa_embed_dim, extent, coord_dim = 2, device = "cuda"):
        """
        Args:
            spa_embed_dim: the output spatial relation embedding dimention
            coord_dim: the dimention of space, 2D, 3D, or other
...
class RBFSpatialRelationEncoder(PositionEncoder):
    """
    ...
    """
    def __init__(self, model_type, train_locs, coord_dim = 2, num_rbf_anchor_pts = 100, rbf_kernal_size = 10e2, rbf_kernal_size_ratio = 0.0, max_radius = 10000,
                rbf_anchor_pt_ids = None, device = "cuda"):
        """
        Args:
